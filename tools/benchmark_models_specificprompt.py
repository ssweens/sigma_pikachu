...
LOCAL_MODELS_TO_TEST = [] #sorted(set(extract_model_paths_from_config())) # if not MODELS_TO_TEST else MODELS_TO_TEST
...
for model in pathlib.Path(MODELS_DIR).rglob("*.gguf"):
    ...
    LOCAL_MODELS_TO_TEST.append(str(relative_path))
...
pprint.pprint(LOCAL_MODELS_TO_TEST)
pprint.pprint(API_MODELS_TO_TEST)
pprint.pprint(MLX_MODELS_TO_TEST)
...
def benchmark_openai_model(client, api_model_name, prompt_content, n_predict=N_PREDICT, first_run=False):
    ...
    try:
        ...
        # print(f"API call completed for {api_model_name}.")
        ...
        # print(f"\t {api_model_name} API | Total Time: {round(total_time, 2)}s | Prompt Tokens: {prompt_tokens} | Completion Tokens: {completion_tokens} | Tokens/Sec: {round(total_tokens_per_second, 2)}")

        return prompt_metrics, generation_metrics, total_time, 0.0  # VRAM is not applicable for API models
        
    except Exception as e:
        print(f"API call failed for {api_model_name}: {e}")
        return None, None, None, None


# --- Function to benchmark MLX models ---
def benchmark_mlx_model(model_name, prompt_content, n_predict=N_PREDICT, first_run=False):
    ...
    try:
        ...
        mlx_cmd = f"python -m mlx_lm generate --model {model_name} --prompt \"$(cat tools/sample_reddit_prompt.md)\" --system-prompt \"\\no_think\" --max-tokens {n_predict} --temp 0.5 --kv-bits 8 --max-kv-size 33000 --chat-template-config '{{\"enable_thinking\":false}}'"
        ...
        # print(f"\t {model_name} MLX | total: {total_time} s | prompt: {prompt_tokens} tokens in {prompt_tokens/prompt_tps} s ({prompt_tps} tps) | generation: {generation_tokens} tokens in {generation_tokens/generation_tps} s ({generation_tps} tps)")
        ...
        return prompt_metrics, generation_metrics, total_time, peak_vram_usage_gb

    except Exception as e:
        print(f"MLX generation failed for {model_name}: {e}")
        return None, None, None, None


# --- Function to benchmark local models ---
def benchmark_local_model(model_path, prompt_content, n_predict=N_PREDICT, first_run=False):
    ...
    # print(f"\t {model_path} model | total: {total_time} s | prompt: {prompt_metrics['tokens']} tokens in {prompt_metrics['time']} s ({prompt_metrics['tps']} tps) | generation: {generation_metrics['tokens']} tokens in {generation_metrics['time']} s ({generation_metrics['tps']} tps) | peak VRAM: {peak_vram_usage_gb:.3f} GB")
...
def calc_average_metrics(runs):
    """Calculates the average metrics from a list of runs."""
    if not runs:
        return None

    num_runs = len(runs)
    avg_prompt_time = sum(r[0]['time'] for r in runs) / num_runs
    avg_prompt_tokens = sum(r[0]['tokens'] for r in runs) / num_runs
    avg_prompt_tps = sum(r[0]['tps'] for r in runs) / num_runs
    avg_gen_time = sum(r[1]['time'] for r in runs) / num_runs
    avg_gen_tokens = sum(r[1]['tokens'] for r in runs) / num_runs
    avg_gen_tps = sum(r[1]['tps'] for r in runs) / num_runs
    avg_total_time = sum(r[2] for r in runs) / num_runs
    avg_peak_vram = sum(r[3] for r in runs) / num_runs

    return {
        'prompt': {'time': avg_prompt_time, 'tokens': avg_prompt_tokens, 'tps': avg_prompt_tps},
        'generation': {'time': avg_gen_time, 'tokens': avg_gen_tokens, 'tps': avg_gen_tps},
        'total': {'time': avg_total_time, 'vram_gb_max': avg_peak_vram}
    }
...
for mlx_model in MLX_MODELS_TO_TEST:
    model_runs = []
    for i in range(n_mlx_runs):
        run_metrics = benchmark_mlx_model(mlx_model, BENCHMARK_PROMPT, n_predict=1024, first_run=(i == 0))
        if run_metrics:
            model_runs.append(run_metrics)
            # After each successful run, calculate and store the new average
            mlx_model_metrics[mlx_model] = calc_average_metrics(model_runs)
            # Print updated table
            print_benchmark_table(local_model_metrics, mlx_model_metrics, api_model_metrics)

# --- Benchmark Local Models ---
print("\n--- Benchmarking Local Models ---")
for model in LOCAL_MODELS_TO_TEST:
    model_runs = []
    for i in range(n_local_runs):
        run_metrics = benchmark_local_model("/users/ssweens/models/" + model, BENCHMARK_PROMPT, n_predict=1024, first_run=(i == 0))
        if run_metrics:
            model_runs.append(run_metrics)
            local_model_metrics[model] = calc_average_metrics(model_runs)
            print_benchmark_table(local_model_metrics, mlx_model_metrics, api_model_metrics)
...
for api_model in API_MODELS_TO_TEST:
    model_runs = []
    for i in range(n_api_runs):
        run_metrics = benchmark_openai_model(client, api_model, BENCHMARK_PROMPT, n_predict=1024, first_run=(i == 0))
        if run_metrics:
            model_runs.append(run_metrics)
            api_model_metrics[api_model] = calc_average_metrics(model_runs)
            print_benchmark_table(local_model_metrics, mlx_model_metrics, api_model_metrics)
